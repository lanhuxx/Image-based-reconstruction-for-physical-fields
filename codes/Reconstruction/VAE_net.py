...
def encoder(image, gf_dim=32, reuse=False, name="generator"):
	...
	with tf.variable_scope(name):
		...
		e5 = batch_norm(conv2d(input_=lrelu(e4), output_dim=gf_dim*16, kernel_size=4, stride=2, name='g_e5_conv'), name='g_bn_e5')
		# #第六个卷积层，输出尺度[1, 4, 4, 512]
		e6 = batch_norm(conv2d(input_=lrelu(e5), output_dim=gf_dim, kernel_size=4, stride=2, name='g_e6_conv'), name='g_bn_e6')
		# #第七个卷积层，输出尺度[1, 2, 2, 512]
		# e7 = batch_norm(conv2d(input_=lrelu(e6), output_dim=gf_dim*1, kernel_size=4, stride=2, name='g_e7_conv'), name='g_bn_e7')
		...
		mean_b = tf.get_variable('biases1', [feature_num], initializer=tf.constant_initializer(0.1))
		sigma_sq_w = tf.get_variable('weights2', [nodes, feature_num])
		sigma_sq_b = tf.get_variable('biases2', [feature_num], initializer=tf.constant_initializer(0.1))

		mean = tf.matmul(e7, mean_w) + mean_b
		sigma_sq = tf.matmul(e7, sigma_sq_w) + sigma_sq_b
		return mean, sigma_sq

def decoder(e, gf_dim=32, reuse=False, name="generator"):
	...
	with tf.variable_scope(name):
		...
		# e8 = batch_norm(conv2d(input_=lrelu(e7), output_dim=gf_dim*1, kernel_size=4, stride=2, name='g_e8_conv'), name='g_bn_e8')
		...
		# d2 = deconv2d(input_=tf.nn.relu(e), output_dim=gf_dim*4, kernel_size=4, stride=2, name='g_d2')
		...
		d3 = deconv2d(input_=tf.nn.relu(e), output_dim=gf_dim*16, kernel_size=4, stride=2, name='g_d3')
		...
		d8 = deconv2d(input_=tf.nn.relu(d7), output_dim=3, kernel_size=4, stride=2, name='g_d8')
