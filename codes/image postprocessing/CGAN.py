...
parser.add_argument("--ckpt_dir", default='./ckpt/decoded_400_features=1', help="path of snapshots") #保存模型的路径
parser.add_argument("--out_dir", default='./train_out_decoded_400_features=1', help="path of train outputs") #训练时保存可视化输出的路径
...
parser.add_argument("--save_pred_every", type=int, default=500, help="times to save.") #训练中每过多少step保存模型(可训练参数)
...
parser.add_argument("--train_picture_path", default='E:/backups/stamping/images/decoded_samples=400_features=1/', help="path of training datas.") #网络训练输入的图片路径
parser.add_argument("--train_label_path", default='E:/backups/stamping/images/stamping_/', help="path of training labels.") #网络训练输入的标签路径
...
def main(): #训练程序的主函数
	if not os.path.exists(args.ckpt_dir): #如果保存模型参数的文件夹不存在则创建
	...
	summary_writer = tf.summary.FileWriter(args.ckpt_dir, graph=tf.get_default_graph()) #日志记录器
	...
	for epoch in range(args.epoch): #训练epoch数
		...
		for step in range(len(train_picture_list)): #每个训练epoch中的训练step数
			...
			print('epoch\t {:d} \tstep\t {:d} \t gen_loss =\t {:.6f} \tdis_loss =\t {:.6f}'.format(epoch, step, gen_loss_value, dis_loss_value))
main()
...
def images_by_CGAN():
	...
	with tf.Session() as sess:
		...
		dir_ = args.ckpt_dir + '/'
		module_file = tf.train.latest_checkpoint(dir_)
		saver.restore(sess, module_file)

		for step in range(801):
			tpl = 'E:/backups/impact2/punching press/xOy/images/decoded_801_features=1/%d.jpg' % int(step+1)
			picture_name, _ = os.path.splitext(os.path.basename(tpl))
			# print(picture_name)
			picture_resize, label_resize, picture_height, picture_width, label_height, label_width = ImageReader2(
				file_name=picture_name, picture_path=args.train_picture_path, label_path=args.train_label_path,
				picture_format = args.train_picture_format, label_format = args.train_label_format, size = args.image_size)
			# print(picture_resize)
			# print(label_resize)
			...
			gen_label_value = sess.run(gen_label, feed_dict=feed_dict) #run出生成器的输出
			# print(gen_label_value)
			write_image = get_write_picture_single(gen_label_value, label_height, label_width)
			write_image_name = 'CGAN_801_feature=1/'+ str(int(step+1)) + ".jpg" #待保存的训练可视化结果路径与名称
			cv2.imwrite(write_image_name, write_image) #保存训练的可视化结果
			print(step)
# images_by_CGAN()
